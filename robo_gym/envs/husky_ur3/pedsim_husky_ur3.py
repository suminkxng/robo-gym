...
class Husky_ur3_Env(gym.Env):
    ...
    def __init__(self, rs_address=None, **kwargs):
        ...
        self.action_space = spaces.Box(low=np.array(action_low), high=np.full((2), 1.0), dtype=np.float64)
        ...
        self.distance_threshold_human = 1.3
    ...
    def _get_robot_server_state_len(self):
        ...
        # human_poses = [0.0] * 9
        human_pose = [999.0] * 2
        # real_human_poses = [0.0] * 9
        rs_state = target + husky_pose + husky_twist + scan  + [collision] + human_pose  # 3+3+2+542+1+9+1 = 561
...
class PedsimWithHusky_ur3(Husky_ur3_Env):
    ...
    def reset(self, start_pose = None, target_pose = None, human_pose = None):
        """Environment reset.
        ...
        # if len(human_pose) == 3:
        #     rs_state[551:554] = human_pose[0]
        # elif len(human_pose) == 6:
        #     rs_state[554:557] = human_pose[1]
        # elif len(human_pose) == 9:
        #     rs_state[557:560] = human_pose[2]
    ...
    def _reward(self, rs_state, action):
        ...
        human_coords = np.array([rs_state[551],rs_state[552]])
        euclidean_dist_2d = np.linalg.norm(target_coords - husky_coords, axis=-1)
        distance = np.linalg.norm(human_coords - husky_coords, axis=-1)
        # Reward base
        ...
        if not self.real_robot:
            ...
            if (distance < 1.0):
                ...
                info['final_status'] = 'collision'
                print("Too_close_to_human")

            if (distance < self.distance_threshold_human):
                print("distance: ", distance)
                reward = (distance - self.distance_threshold_human) * self.discomfort_penalty_factor * self.time_step * 100
                done = False
                # info['final_status'] = 'collision'
                print("Discomfort_distance")
    ...
    def _get_human_pose(self, rs_state):
        
        # if len(rs_state) > 550:
        # human_pose = np.array([rs_state[551], rs_state[552]])

        #     if len(rs_state) > 553:
        #         human_pose_1 = np.array([rs_state[554], rs_state[555]])
        #         print("human_pose_1: ", human_pose_1)

        #         if len(rs_state) > 556:
        #             human_pose_2 = np.array([rs_state[557], rs_state[558]])
        #             print("human_pose_2: ", human_pose_2)

        
        # print("final_human_pose", human_pose_0, human_pose_1, human_pose_2)
        human_pose = np.array([rs_state[551], rs_state[552]])
